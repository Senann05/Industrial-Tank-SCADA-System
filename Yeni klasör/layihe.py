...
# --- PORT VƏ FİZİKİ PARAMETRLƏR ---
SERIAL_PORT = 'COM5' 
BAUD_RATE = 9600

# Çənin həndəsi və fiziki parametrləri
TANK_H = 17.0        # Ümumi hündürlük (sm)
R_BOTTOM = 4.0       # Aşağı radius (sm)
R_TOP = 5.0          # Yuxarı radius (sm)
TEMP = 35.0          # Neftin temperaturu (C)
RHO_20 = 850.0       # 20 dərəcədəki standart sıxlıq (kq/m^3)
BETA = 0.73          # Temperatur əmsalı
...
class ProfessionalScada:
    def __init__(self, root):
        ...
        # Vizual olaraq kəsik konus forması (aşağı dar, yuxarı geniş)
    ...
    # --- KÜTLƏ HESABLANMASI (FİZİKİ DÜSTURLAR) ---
    def calculate_mass(self, percentage):
        if percentage <= 0: return 0.0
        
        # 1. Hündürlüyün tapılması (sm)
        h = (percentage / 100.0) * TANK_H
        
        # 2. Maye səthinin radiusunun tapılması
        r_h = R_BOTTOM + (h / TANK_H) * (R_TOP - R_BOTTOM)
        
        # 3. Həcmin tapılması (sm^3)
        v_cm3 = (math.pi * h / 3.0) * (R_BOTTOM**2 + R_BOTTOM * r_h + r_h**2)
        
        # 4. Temperatur fərqinə görə sıxlığın korreksiyası (pt = p20 - b(t-20))
        rho_t = RHO_20 - BETA * (TEMP - 20.0)
        
        # 5. Kütlənin tapılması (Həcmi m^3-ə çevirərək sıxlığa vururuq)
    ...
    def update_ui(self, percentage):
        ...
        # Radiusa görə x koordinatlarının hərəkəti (aşağı dar, yuxarı geniş)
        x_left_bottom = 120
        x_right_bottom = 180
